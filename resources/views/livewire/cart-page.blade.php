            ...
            <tbody>
                @forelse ($cart_items as $item)
                    <tr wire:key="{{ $item['product_id'] }}"">
                        ...
                        <td class="py-4">{{ Number::currency($item['unit_amount'], 'AOA') }}</td>
                        <td class="py-4">
                        <div class="flex items-center">
                        ...
                        <td class="py-4">{{ Number::currency($item['total_amount'], 'AOA') }}</td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-4 text-4x1 font-semibold text-slate-500">No items available in the cart!</td>
                    </tr>
                @endforelse
              <!-- More product rows -->
            </tbody>
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>{{ Number::currency($grand_total, 'AOA') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxes</span>
            <span>{{ Number::currency(0, 'AOA') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Shipping</span>
            <span>{{ Number::currency(0, 'AOA') }}</span>
          ...
          <div class="flex justify-between mb-2">
            ...
            <span class="font-semibold">{{ Number::currency($grand_total, 'AOA') }}</span>
          </div>
          @if ($cart_items)
